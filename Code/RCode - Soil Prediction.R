...
#Training using partial least squares
set.seed(123)
...
#Training using k-Nearest Neighbour
...
library(nFactors)
ev <- eigen(cor(trainandtest)) # get eigenvalues
ap <- parallel(subject=nrow(trainandtest),var=ncol(trainandtest),
               rep=100,cent=.05)
